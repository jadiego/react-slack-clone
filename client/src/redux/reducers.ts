...
const messagebar = (
  state = model.MessageBarInitialState,
  action: Actions
): model.MessageBarState => {
  switch (action.type) {
...
const channels = (
  state = model.ChannelsInitialState,
  action: Actions
): model.Channel[] => {
  switch (action.type) {
    ...
    case types.WS_CHANNEL_NEW:
      return [...state, ...[action.payload]];

    case types.WS_CHANNEL_UPDATE: {
      let chans = state.slice();
      let i = chans.map(chan => chan.id).indexOf(action.payload.id);
      chans[i] = action.payload;
      return chans;
    }

    case types.WS_CHANNEL_DELETE: {
      let chans = state.slice();
      return chans.filter(chan => chan.id !== action.payload.id);
    }
...
const messages = (
  state = model.MessagesInitialState,
  action: Actions
): model.Message[] => {
  switch (action.type) {
    ...
    case types.WS_MESSAGE_NEW: {
      if (location.pathname.endsWith(action.payload.channelid)) {
        return [...state, action.payload];
      } else {
        return state;
      }

...
const mostMessages = async function(res, data, store, user) {
  ...
  ifChannelExists(channel, res, async function() {
    ...
    res.send(
      `For the ${channel.name} channel, ${user.firstname} ${
        user.lastname
      } made the most posts.`
    );
  });
...
const lateMessage = async function(res, data, store, user) {
  ...
  if (channelname) {
    ...
    ifChannelExists(channel, res, async function() {
      ...
      if (message) {
        res.send(
          `Your last message in the ${channel.name} channel was on ${moment(
            message.createdat
          ).format("LLL")}.`
        );
      } else {
        res.send(
          `You haven't posted a message to this channel yet silly! Try another channel.`
        );
      }
    });
  } else {
    ...
    if (message) {
      res.send(
        `Your last message was on ${moment(message.createdat).format("LLL")}.`
      );
    } else {
      res.send(
        `You haven't posted a message to this channel yet silly! Try another channel.`
...
const countMessages = async function(res, data, store, user) {
  ...
  ifChannelExists(channel, res, async function() {
    if (data.entities.datetime !== undefined) {
      ...
      res.send(
        `On ${moment(date).format("LL")}, you posted ${count} messages in the ${
          channel.name
        } channel.`
      );
    } else {
      ...
      res.send(
        `Overall, you posted ${count} messages in the ${channel.name} channel.`
      );
    }
  });
};
...
const ifChannelExists = function(channel, res, callback) {
  if (channel) {
    ...
    res.send(
      `Sorry I don't quite understand what you meant. Please ask again.`
    );
  }
};
